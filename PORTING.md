...
> **2026-09-23 · 实测校正。** 本文的设计已被实现出一部分（`pc/`，见 `pc/README.md`），
> 实现过程对本文的量化结论做了实测校验，**有 7 处需要修正**（§2.1 的文件分类数字、
> §2.1 的测试资产数字、"125 个文件可直接编进桌面模块"这一条、§2.1 未提的第三方依赖继承、
> §5.7 对 `HotkeyDecision.kt` 的描述、§2.1 遗漏的 i18n 资产、§1 平台矩阵的一处结论）。
> 其中**最要紧的是第三条**：那 125 个文件不是闭集，141 个被标为可移植的文件里
> 57 个引用了同包的非可移植声明，而 Kotlin 的同包引用不带 import，任何依赖扫描都看不见——
> 这会把 P-2 的工期低估一个量级。逐条校正与复现命令见 [`pc/GAP_REPORT.md`](pc/GAP_REPORT.md) §2。
> 本文保留为设计快照，不逐条改写。
